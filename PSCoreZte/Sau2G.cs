...
namespace PSCoreZte
{
    class Sau2G
    {
        ...
        List<SAU2G_Model> sau2GList = new List<SAU2G_Model>();
            ...
            foreach (string file_to_parse in FilesToParse)
            {
                ...
                using (StreamReader sr = File.OpenText(@file_to_parse))
                {
                    ...
                    while ((input = sr.ReadLine()) != null)
                    {
                        ...
                        sau2GList.Add(new SAU2G_Model { maximumAttachedSubscribers = Convert.ToInt32(tokens[11]), resultTime = oDate, nodeName = nodeName });
                        line_count++;
                    }
                    sr.Close();
                }
