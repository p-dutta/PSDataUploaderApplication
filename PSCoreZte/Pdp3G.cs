...
namespace PSCoreZte
{
    class Pdp3G
    {
        ...
        List<Pdp3G_model> dataList = new List<Pdp3G_model>();

        public int parsePdp3GFile()
        {
            int line_count = 0;
            ...
            foreach (string file_to_parse in FilesToParse)
            {
                ...
                using (StreamReader sr = File.OpenText(@file_to_parse))
                {
                    ...
                    while ((input = sr.ReadLine()) != null)
                    {
                        ...
                        Pdp3G_model data = new Pdp3G_model();
                        data.maxNumberOfActivatedPDPContexts = Convert.ToInt32(tokens[26]);
            ...
            foreach (var data in dataList)
            {
                ...
                queryString += "INSERT into ps_sgsn_3g_pdp ( iu_mode_maximum_act_pdp_context,node_name,vendor,result_time) values ('" + data.maxNumberOfActivatedPDPContexts + "','" + data.nodeName + "','" + data.vendor + "','" + data.resultTime.ToString("yyyy-MM-dd HH:mm:ss") + "');";
            }
