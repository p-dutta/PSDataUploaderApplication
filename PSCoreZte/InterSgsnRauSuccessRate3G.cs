...
namespace PSCoreZte
{
    class InterSgsnRauSuccessRate3G
    {
        ...
        List<InterSgsnRauSuccessRate3G_Model> dataList = new List<InterSgsnRauSuccessRate3G_Model>();

        public int parseInterSgsnRauSucRate3GFile()
        {
            int line_count = 0;
            ...
            foreach (string file_to_parse in FilesToParse)
            {
                ...
                using (StreamReader sr = File.OpenText(@file_to_parse))
                {
                    ...
                    while ((input = sr.ReadLine()) != null)
                    {
                        ...
                        InterSgsnRauSuccessRate3G_Model data = new InterSgsnRauSuccessRate3G_Model();
                        data.interSgsnRAUpdateProcedureAttemptedTimes = Convert.ToInt32(tokens[20]);
                        data.interSgsnRAUpdateProcedureSuccessTimes = Convert.ToInt32(tokens[21]);
            ...
            foreach (var data in dataList)
            {
                ...
                queryString += "INSERT into ps_sgsn_3g_inter_sgsn_rau_success_rate ( iu_mode_inter_sgsn_rau_request_times,iu_mode_inter_sgsn_rau_success_times,node_name,vendor,result_time) values ('" + data.interSgsnRAUpdateProcedureAttemptedTimes + "','" + data.interSgsnRAUpdateProcedureSuccessTimes + "','" + data.nodeName + "','" + data.vendor + "','" + data.resultTime.ToString("yyyy-MM-dd HH:mm:ss") + "');";
            }
    ...
    class InterSgsnRauSuccessRate3G_Model
    {
        public int interSgsnRAUpdateProcedureAttemptedTimes { get; set; }
        public int interSgsnRAUpdateProcedureSuccessTimes { get; set; }
        public DateTime resultTime { get; set; }
        public string vendor { get; set; }
        public string nodeName { get; set; }

        public InterSgsnRauSuccessRate3G_Model()
        {
            vendor = "zte";
        }
