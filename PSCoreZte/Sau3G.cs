...
namespace PSCoreZte
{
    class Sau3G
    {
        ...
        List<SAU3G_Model> sau3GList = new List<SAU3G_Model>();

        public int parseSau3GFile()
        {
            int line_count = 0;
            ...
            foreach (string file_to_parse in FilesToParse)
            {
                ...
                using (StreamReader sr = File.OpenText(@file_to_parse))
                {
                    ...
                    while ((input = sr.ReadLine()) != null)
                    {
                        ...
                        sau3GList.Add(new SAU3G_Model { maximumAttachedSubscribers = Convert.ToInt32(tokens[10]), resultTime = oDate, nodeName = nodeName });
                        line_count++;
                    }
                    sr.Close();
                }
            ...
            string queryString = "";
            foreach (var sau3g in sau3GList)
            {
                queryString += "INSERT into ps_sgsn_3g_sau ( iu_mode_maximum_attached_users,node_name,vendor,result_time) values ('" + sau3g.maximumAttachedSubscribers + "','" + sau3g.nodeName + "','" + sau3g.vendor + "','" + sau3g.resultTime.ToString("yyyy-MM-dd HH:mm:ss") + "');";
            }
    ...
    class SAU3G_Model
    {
        public int maximumAttachedSubscribers { get; set; }
