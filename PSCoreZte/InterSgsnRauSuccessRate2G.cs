...
namespace PSCoreZte
{
    class InterSgsnRauSuccessRate2G
    {
        ...
        List<InterSgsnRauSuccessRate2G_Model> dataList = new List<InterSgsnRauSuccessRate2G_Model>();

        public int parseInterSgsnRauSucRateFile()
        {
            int line_count = 0;
            ...
            foreach (string file_to_parse in FilesToParse)
            {
                ...
                using (StreamReader sr = File.OpenText(@file_to_parse))
                {
                    ...
                    while ((input = sr.ReadLine()) != null)
                    {
                        ...
                        InterSgsnRauSuccessRate2G_Model data = new InterSgsnRauSuccessRate2G_Model();
                        data.interSgsnRAUpdateProcedureAttemptedTimes = Convert.ToInt32(tokens[15]);
                        data.interSgsnRAUpdateProcedureSuccessTimes = Convert.ToInt32(tokens[14]);
            ...
            foreach (var data in dataList)
            {
                ...
                queryString += "INSERT into ps_sgsn_2g_inter_sgsn_rau_success_rate ( gb_mode_inter_sgsn_rau_request_times,gb_mode_inter_sgsn_rau_success_times,node_name,vendor,result_time) values ('" + data.interSgsnRAUpdateProcedureAttemptedTimes + "','" + data.interSgsnRAUpdateProcedureSuccessTimes + "','" + data.nodeName + "','" + data.vendor + "','" + data.resultTime.ToString("yyyy-MM-dd HH:mm:ss") + "');";
            }
    ...
    class InterSgsnRauSuccessRate2G_Model
    {
        ...
        public InterSgsnRauSuccessRate2G_Model()
        {
            vendor = "zte";
        }
