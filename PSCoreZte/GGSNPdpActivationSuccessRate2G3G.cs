...
namespace PSCoreZte
{
    class GGSNPdpActivationSuccessRate2G3G
    {
        ...
        List<GGSNPdpActivationSuccessRate2G3G_Model> dataList = new List<GGSNPdpActivationSuccessRate2G3G_Model>();

        public int parseGgsnPdpActiSucRate2G3GFile()
        {
            int line_count = 0;
            ...
            foreach (string file_to_parse in FilesToParse)
            {
                ...
                using (StreamReader sr = File.OpenText(@file_to_parse))
                {
                    ...
                    while ((input = sr.ReadLine()) != null)
                    {
                        ...
                        GGSNPdpActivationSuccessRate2G3G_Model data = new GGSNPdpActivationSuccessRate2G3G_Model();
                        data.timesOfPdpContextActivationRequest = Convert.ToInt32(tokens[9]);
                        data.SuccessfulTimes = Convert.ToInt32(tokens[10]);
            ...
            foreach (var data in dataList)
            {
                ...
                queryString += "INSERT into ps_ggsn_2g_3g_pdp_acti_suc_ratio ( pdp_context_act_requests,pdp_context_act_success,node_name,vendor,result_time) values ('" + data.timesOfPdpContextActivationRequest + "','" + data.SuccessfulTimes + "','" + data.nodeName + "','" + data.vendor + "','" + data.resultTime.ToString("yyyy-MM-dd HH:mm:ss") + "');";
            }
    ...
    class GGSNPdpActivationSuccessRate2G3G_Model
    {
        public int timesOfPdpContextActivationRequest { get; set; }
        public int SuccessfulTimes { get; set; }
        public DateTime resultTime { get; set; }
        public string vendor { get; set; }
        public string nodeName { get; set; }

        public GGSNPdpActivationSuccessRate2G3G_Model()
        {
            vendor = "zte";
        }
